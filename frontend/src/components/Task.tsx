...
import { DeepPartial, TodoTaskType, UserType } from '../types';
...
export const Task = ({ user, tasks }: { user: DeepPartial<UserType>; tasks: Partial<TodoTaskType[]> }) => {
	...
	const [selectedTask, setSelectedTask] = useState<Partial<TodoTaskType>>({});

	const tasksCategory = (() => {
		const pathname = location.pathname as '/' | '/personal' | '/work' | '/groups';
		switch (pathname) {
			case '/':
				return 'Tasks';
			case '/personal':
	...
	const handleOpenModal = (taskId?: number) => {
		if (taskId) {
			setSelectedTask(tasks.find((task) => task?.id === taskId) as TodoTaskType);
			setFormType('edit');
			setHandleOpen(true);
		} else {
			setFormType('create');
