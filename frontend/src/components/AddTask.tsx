...
import { Subtasks, TodoTaskType } from '../types';
...
export const AddTask = ({
	...
	selectedTask: Partial<TodoTaskType>;
	...
	const [form, setForm] = useState<Partial<TodoTaskType>>({
		...initialState
	});
	const [taskValues, setTaskValues] = useState<string>('');

	const setSubtasks = (subtasks: Subtasks[]) => {
	...
	const handleCheckedSubtasks = (event: React.ChangeEvent<HTMLInputElement>) => {
		...
		setSubtasks(updatedIsDoneTask as Subtasks[]);
	};
	...
	const handleUpdate = async (event: React.FormEvent) => {
		event.preventDefault();
		try {
			const response = await apiCall(`/todo/${selectedTask.id}`, 'PATCH', form);
			await checkToken(response);
			notification.success(response.message);
		} catch (error) {
			notification.error((error as Error).message || 'Network error');
			...
			<div className="flex h-96 w-96 flex-col overflow-auto rounded-lg bg-gray-800 p-5">
				...
				<div className="mb-5 flex items-center justify-between">
					...
					<select
						...
						onChange={setFormValues}>
						{formType === 'edit' && selectedTask.taskType && (
							<option disabled>Current: {selectedTask.taskType}</option>
						)}
				...
				<div className="mb-4">
					...
					{Array.from(form.subtasks ?? []).map((subtask: Subtasks, index: number) => (
						<div
							key={index}
				...
				<button
					...
					{formType === 'edit' ? 'Save Changes' : 'Create Task'}
				</button>
			</div>
		</div>
	);
